...
const namedType = (name) => ({
  kind: 'NamedType',
  name: { kind: 'Name', value: name }
})

const nonNullNamedType = (name) => nonNullType(namedType(name))

const nonNullListOfNamedType = (name) =>
...
export default function transformSchema (ast, relay) {
  let types = new Map()
  let queries = []
  let mutations = []
  let defs = []
